...
export async function POST(request: Request) {
  try {
    ...
    const bytes = await pdf.save(); return new Response(bytes, { headers: { "Content-Type": "application/pdf", "Content-Disposition": `attachment; filename="${caseRecord.id}-draft-affidavit.pdf"`, "Cache-Control": "no-store" } });
